...
full_dataset = CustomFinetuneDataset(
    full_df,
    np.concatenate([graph[train_df.index], graph[val_df.index]]),
    np.concatenate([text[train_df.index], text[val_df.index]]),
    np.concatenate([kg[train_df.index], kg[val_df.index]]),
...
weights_path = "/media/ubuntu/8TB/g3-bbm-project/checkpoints/epoch_090/model.safetensors"
state_dict = load_file(weights_path)
pretrained_model.load_state_dict(state_dict, strict=False)
pretrained_model.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
def plot_umap_embeddings(model, dataset, title_prefix="Fine-tuned", dataset_name="HIV"):
    ...
    inactive_color = palette[1]  # kırmızı
    active_color = palette[0]    # mavi

    # Plot
    plt.figure(figsize=(7.5, 6.5))  # Daha geniş grafik
    ...
    plt.grid(False)
    plt.tight_layout(rect=[0, 0, 1, 0.97])  # Başlık için ekstra alan bırak
...
class MultimodalClassificationModel(nn.Module):
    def __init__(self, pretrained_model, hidden_size=768, num_labels=1, dropout_prob=0.2):
        super().__init__()
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
fine_model = MultimodalClassificationModel(pretrained_model)
